...
This repository includes an opt-in-at-runtime, enabled-by-default performance layer for the installed `chatterbox-tts==0.1.6` Turbo path. It is deliberately limited to changes intended to preserve generated speech-token IDs and waveform bytes for fixed seeds and settings.
...
The performance layer does **not** enable:
...
Those are separate numerical- or quality-parity experiments and must not be mixed into the exact-output rollout.

## Compatibility safety

The source-sensitive rewrites require both:

1. the expected installed distribution version, defaulting to `chatterbox-tts==0.1.6`; and
...
| `TURBO_EXPECTED_CHATTERBOX_VERSION` | `0.1.6` | Package-version guard |
...
- confirmation that SDPA, compilation, and microbatching remain disabled.

Example:

```bash
curl -s http://127.0.0.1:7766/status | jq .performance
```
...
The command exits non-zero when speech-token, float-waveform, PCM16, sample-rate, or sample-count parity differs. The three generated reports are:
...
5. Deploy to one worker with all experimental backends still disabled.
6. Confirm cache counters and latency through `/status` and normal telemetry.
7. Roll out to the remaining workers only after no profile-specific regression appears.

## Rollback

Disable the layer without changing code:
